...
TOKEN = '5160604243:AAE6Qs-_21_3325ONlKIpO-gybcsYQzaqlA'
...
def inline_keyboard_handler(update, context):
    ...
    message_text = update.message.text   ##type in message
    # logging.info(message_text)
    serial_info = search_serial(message_text)
    global data_set
    data_set = serial_info
    keyboard = []
    # print(serial)
    for serial in serial_info:
        # logging.info(serial["show"]["image"])
        # logging.info("hihi")
        temp = serial["show"]["id"]
        # img = serial["show"]["image"]["medium"]
        # img = serial["show"]["language"]
        # img = str(serial["show"]["image"]["medium"])
        logging.info(serial)
        # logging.info(serial["show"]["image"]["medium"])
        # keyboard.append([InlineKeyboardButton(f'{serial["show"]["name"]}', callback_data=[temp, img])])
        keyboard.append([InlineKeyboardButton(f'{serial["show"]["name"]}', callback_data=temp)])
...
def search(data, id):
    ...
    for tv in range(l):
        # print(tv["show"]["id"])
        if data[tv]["show"]["id"] == id:
            text = data[tv]["show"]["image"]["medium"]
            print(text)
            return text
...
def button(update, context):
    ...
    text = search(data_set, query.data)
    logging.info("finish")
